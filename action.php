...
$query_array = mysql_fetch_assoc($result);
    if($password==$query_array['password']){
        ...
        $_SESSION['semester']=$query_array['semester'];
...
if(session_start() && !(isset ($_SESSION['username']) && isset ($_SESSION['pass']))){
    ...
    if (!($_POST['userName'])=="" && !($_POST['password'])=="" && !($_POST['confirmPassword'])=="" && !($_POST['securityQuestion'])=="" && !($_POST['securityAnswer'])=="" && !($_POST['eMail'])=="") {
        ...
        if($password!=$confirmPassword) {
            ...
            $errorDuplicateUser="Username Entered Already Exists";
            header("Location: errorPage.php?message=$errorDuplicateUser");
        } else if(enrollAlreadyExists($enrollment)) {
            $errorDuplicateUser="Enrollment Number Entered Already Exists";
            header("Location: errorPage.php?message=$errorDuplicateUser");
        } else {
			...
			$sql="INSERT INTO users(enrollment, name, branch, username, password, security_question, securityAnswer, email, semester) VALUES ('$enrollment', '$name','$branch', $semester, '$username', '$password', '$securityQuestion', '$securityAnswer', '$eMail', '$semester')";
			}
            require 'connection.php';
...
function initializeExam() { // op=3
    if (session_start() && !(isset ($_SESSION['username']) && isset ($_SESSION['pass']))) {
        header("Location: login.php");
    } else if (isset($_SESSION['examid']) && isset($_SESSION['examcode'] ) && isset($_SESSION['timelimit'])) {
		//header("Location: paper.php?qNo=1");
	} else if (isset($_POST['examid']) && $_POST["examid"]!="") {
	...
	if($result["examcode"]!=$examcode){
		$message ="Error no. 110 -Enter an Invalid/Outdated Examcode !";
		header("Location: errorPage.php?message=$message");
	}
	...
	$category=$_SESSION['examName']=$result['examName'];
	$_SESSION['examcode']=$result['examcode'];
    //$_SESSION['day']= date('D');
    ...
    $_SESSION['timelimit']=$result['timelimit']*60+time();
    $_SESSION['maxque']=$result['maxque'];
	$sql="SELECT category FROM questions WHERE category='$category'";
	$result = mysql_query($sql) or die(mysql_error());
	$res=mysql_num_rows($result) or die(mysql_error());
	if($res==0) {
    ...
    $i=1;
	//echo $_SESSION['maxque'];
    while ($i<=$_SESSION['maxque']) {
        ...
        $que=  mysql_fetch_assoc($result);
            ...
            $_SESSION[$str]['opt']="0";
			//echo $str.")".$_SESSION[$str]['question']."<br />".$_SESSION[$str]['op1']." ".$_SESSION[$str]['op2']." ".$_SESSION[$str]['op3']." ".$_SESSION[$str]['op4'];
        $i++;
    }
        header("Location: paper.php?qNo=1");
    }
} else {
	$message="Error No. 103 - Please enter a valid exam code, Contact Admin to Solve this Issue !";
	header("Location: errorPage.php?message=$message");
}
mysql_close($dbc);
}
...
function quitTest() { //op=9
	if (session_start() && isset($_SESSION["username"]) && isset($_SESSION["pass"]) && isset($_SESSION["examcode"])) {
		...
		$score1=$res["result"];
		$score2=$_SESSION['examName']."=".$score."/".$_SESSION['maxque'];
